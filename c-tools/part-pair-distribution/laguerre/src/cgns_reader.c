...
void main_read_input(void)
{
  ...
  fret = fscanf(infile, "Laguerre Order %d\n", &laguerreOrder);
  fret = fscanf(infile, "\n");
...
void write_avg_reconstruct(void)
{
  ...
  sprintf(fname, "%s/%s/%s", ANALYSIS_DIR, DATA_DIR, OUTPUT_FILE);
