...
void eval_l0(int ll)
{
  ...
  for (int rr = 0; rr < nPointsR; rr++) {
    for (int th = 0; th < nPointsTh; th++) {
      ...
      g_ces[cc] += ell_const*p_ell*g_l0[cl]/r_ab;
    }
  }
}
...
void eval_ln(int ll, int nn)
{
  ...
  for (int rr = 0; rr < nPointsR; rr++) {
    ...
    for (int th = 0; th < nPointsTh; th++) {
      ...
      // Add even
      g_ces[cc] += ell_const*p_ell*weight*g_ln_even[stride]*cos(k_enn*(rEval - 2*meanR))/r_ab;

      // Add odd
      g_ces[cc] += ell_const*p_ell*weight*g_ln_odd[stride]*sin(k_enn*(rEval - 2*meanR))/r_ab;
    }
  }
}
