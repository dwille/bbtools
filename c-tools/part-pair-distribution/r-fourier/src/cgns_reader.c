...
void write_coeffs(int in, int ll, int nn)
{
  ...
  sprintf(fnameEven, "%s/%s/coeffs-r-odd-ll%d-nn%d", ROOT_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
  sprintf(fnameOdd, "%s/%s/coeffs-r-even-ll%d-nn%d", ROOT_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
...
void write_reconstruct(void)
{
  ...
  sprintf(fnameall2, "%s/%s/part-pair-r-%s", ROOT_DIR, DATA_DIR, format);
...
void write_avg_reconstruct(void)
{
  // Set up the filename
  char fname[CHAR_BUF_SIZE] = "";
  sprintf(fname, "%s/%s/part-pair-r-avg-ll-%d-nn-%d", ROOT_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
