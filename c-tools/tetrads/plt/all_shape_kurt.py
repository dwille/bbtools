...
for ii in np.arange(nSims):
  ...
  # kurt
  RoG[ii].kurt   = np.zeros(np.size(tmpT))
  EVar[ii].kurt  = np.zeros(np.size(tmpT))
  Shape[ii].kurt = np.zeros(np.size(tmpT))
  I1[ii].kurt    = np.zeros(np.size(tmpT))
  I2[ii].kurt    = np.zeros(np.size(tmpT))
  I3[ii].kurt    = np.zeros(np.size(tmpT))

  RoG[ii].kurt   = np.genfromtxt(statkurt, skip_header=1, usecols=1)
  EVar[ii].kurt  = np.genfromtxt(statkurt, skip_header=1, usecols=2)
  Shape[ii].kurt = np.genfromtxt(statkurt, skip_header=1, usecols=3)
  I1[ii].kurt    = np.genfromtxt(statkurt, skip_header=1, usecols=4)
  I2[ii].kurt    = np.genfromtxt(statkurt, skip_header=1, usecols=5)
  I3[ii].kurt    = np.genfromtxt(statkurt, skip_header=1, usecols=6)
...
for ii in np.arange(nSims):
  rg_ax.semilogx(data[ii].tau, RoG[ii].kurt, color=colors[ii], alpha=shades[ii])

rg_ax.set_xlabel(r"$t\langle w_f \rangle/a$")
rg_ax.set_ylabel(r"$\mathrm{Kurt}[R_g] - 3$")
#rg_ax.legend(legendText, ncol=1, loc='center left', bbox_to_anchor=(1.10,0.5))

# Save
imgname = imgdir + "all_shape_kurt_rog"
...
for ii in np.arange(nSims):
  v_ax.semilogx(data[ii].tau, EVar[ii].kurt, color=colors[ii], 
    alpha=shades[ii])

v_ax.set_xlabel(r"$t\langle w_f \rangle/a$")
v_ax.set_ylabel(r"$\mathrm{Kurt}[\Delta] - 3$")
#v_ax.legend(legendText, ncol=1, loc='center left', bbox_to_anchor=(1.10,0.5))

# Save
imgname = imgdir + "all_shape_kurt_var"
plt.savefig(imgname + ".png", bbox_inches='tight', format='png')
plt.savefig(imgname + ".pdf", bbox_inches='tight', format='pdf')
...
for ii in np.arange(nSims):
  s_ax.semilogx(data[ii].tau, Shape[ii].kurt, color=colors[ii],
    alpha=shades[ii])

s_ax.set_xlabel(r"$t\langle w_f \rangle/a$")
s_ax.set_ylabel(r"$\mathrm{Kurt}[S] - 3$")
#s_ax.legend(legendText, ncol=1, loc='center left', bbox_to_anchor=(1.10,0.5))

# Save
imgname = imgdir + "all_shape_kurt_sf"
plt.savefig(imgname + ".png", bbox_inches='tight', format='png')
plt.savefig(imgname + ".pdf", bbox_inches='tight', format='pdf')
...
for ii in np.arange(nSims):
  i_ax.semilogx(data[ii].tau, I1[ii].kurt, color=colors[ii],
    alpha=shades[ii])
  i_ax.semilogx(data[ii].tau, I2[ii].kurt, color=colors[ii],
    alpha=shades[ii])
  i_ax.semilogx(data[ii].tau, I3[ii].kurt, color=colors[ii],
    alpha=shades[ii])

#i_ax.text(1000, -4, 'I1')
#i_ax.text(2500, -4, 'I2')
#i_ax.text(6000, 10, 'I3')
i_ax.set_xlabel(r"$t\langle w_f \rangle/a$")
i_ax.set_ylabel(r"$\mathrm{Kurt}[I_j] - 3$")
#i_ax.legend(legendText, ncol=1, loc='center left', bbox_to_anchor=(1.10,0.5))

# Save
imgname = imgdir + "all_shape_kurt_normi"
plt.savefig(imgname + ".png", bbox_inches='tight', format='png')
plt.savefig(imgname + ".pdf", bbox_inches='tight', format='pdf')
