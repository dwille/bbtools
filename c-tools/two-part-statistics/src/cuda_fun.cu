...
{
  threads[0] = MAX_THREADS_1D;
  blocks[0] = (int) ceil((double) length / (double) threads[0]);
  if (threads[0] > length) {
    threads[0] = length;
    blocks[0] = 1;
  }
...
void cuda_find_pairs()
{
  ...
  int threads, blocks;
  cuda_init_threads_blocks(&threads, &blocks, nparts);
  dim3 dimBlocks(threads); dim3 numBlocks(blocks);
  ...
  if (nparts < NPOINTS) {
    ...
    int threads_nb, blocks_nb;
    cuda_init_threads_blocks(&threads_nb, &blocks_nb, nBins);
    dim3 dimBlocks_nb(threads_nb); dim3 numBlocks_nb(blocks_nb);
    ...
    int *_partI;            // array containing first particle
    int *_partJ;            // array containing second particle
    int *_keepIJ;           // 1 or zero, if |r_ij| <= |r0|
    int *_iMatches;         // number of pairs for particle i (not include j=i)
    int *_initFlipCount;    // whether initialized as flipped or no
    cudaMalloc((void**) &_partI, nMax*nparts*sizeof(int));
    cudaMalloc((void**) &_partJ, nMax*nparts*sizeof(int));
    cudaMalloc((void**) &_keepIJ, nMax*nparts*sizeof(int));
    cudaMalloc((void**) &_iMatches, nparts*sizeof(int));
    cudaMalloc((void**) &_initFlipCount, 3*nMax*nparts*sizeof(int));

    printf("\tFinding possible pair permutations... ");
    find_nodes<<<numBlocks, dimBlocks>>>(_parts, nparts, _dom, bc, _binStart,
      _binEnd, _partBin, _partInd, _binDom, _partI, _partJ, _keepIJ, _iMatches,
      _initFlipCount, nMax, R0_a, eps_a, rmax);
...
//    printf("Found %d.\n", nPermutations);
    ...
    // Total number of permutations -- sum keepIJ
    thrust::device_ptr<int> ptr_keepIJ(_keepIJ);
    int nPermutations = thrust::reduce(ptr_keepIJ, ptr_keepIJ + nparts*nMax);
    printf("Found %d.\n", nPermutations);

    // Find number of pairs for each I -- this will allow us to stride it later
    // Do this by summing keepIJ for each i (over nMax bins)
    int *_strides;
    cudaMalloc((void**) &_strides, nparts*sizeof(int));
    find_strides<<<numBlocks_nodes, dimBlocks_nodes>>>(_keepIJ, _strides, 
      nparts, nMax);
...
void cuda_pair_malloc(void)
{
  ...
  cudaMalloc((void**) &(_rSep), sizeof(double) * nPairs);
...
{ 
  ...
  pair_statistics<<<numBlocks_pairs, dimBlocks_pairs>>>(_parts, _pairs, _dom, _rSep, 
    nPairs, tt);
...
//  }
  ...
  cudaMemcpy(rSep, _rSep, sizeof(double) * nPairs, cudaMemcpyDeviceToHost);
...
//    _w_g1, _w_g2, _w_g3, _w_s1, _w_s2, _w_s3);
  ...
  // Calculate higher stat moments of rSep, EVar, Shape, and Sii
  cuda_higher_moments(_rSep, nPairs, m_rSep);
...
void cuda_higher_moments(double *_array, int length, double *moments)
{
  ...
  double *_sum2;   // sum (xi^2)
  double *_skew;   // (xi - x_bar)^3
  double *_kurt;   // (xi - xbar)^4
  cudaMalloc((void**) &(_sum2), sizeof(double) * nPairs); 
  cudaMalloc((void**) &(_skew), sizeof(double) * nPairs); 
  cudaMalloc((void**) &(_kurt), sizeof(double) * nPairs); 
  ...
  higher_moments_kernel<<<numBlocks, dimBlocks>>>(_array, mean, length,
    _sum2, _skew, _kurt); //_diff, _diff2,
  
  // cuda ptr to arrays
  thrust::device_ptr<double> ptr_sum2(_sum2);
  thrust::device_ptr<double> ptr_skew(_skew);
  thrust::device_ptr<double> ptr_kurt(_kurt);

  // Parallel reduce arrays
  ms = iN*thrust::reduce(ptr_sum2, ptr_sum2 + length);
...
//  Alternate definition of higher order moments
//  double ep, var, sdev;
//  double *_diff;   // xi - x_bar
//  double *_diff2;  // (xi - x_bar)^2
//  cudaMalloc((void**) &(_diff), sizeof(double) * nPairs); 
//  cudaMalloc((void**) &(_diff2), sizeof(double) * nPairs);
