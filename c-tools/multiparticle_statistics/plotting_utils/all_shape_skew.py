...
rgFig.suptitle('Skewness Radius of Gyration', fontsize=16)
rg_ax = rgFig.add_subplot(111)
for pp in range(4):
  for dd in range(4):
    i = 4*pp + dd
    rg_ax.semilogx(data[i].time, RoG[i].skew, linewidth=3,
...
rg_ax.legend(legendText, ncol=2,loc='upper left', framealpha=0.9)
rg_ax.set_xlabel("Time [ms]", fontsize=16)
rg_ax.set_ylabel(r"$R$ skewness", fontsize=16)

# EVar
vFig = plt.figure(figsize=(12,8))
vFig.suptitle('Skewness of Eigenvalue variance', fontsize=16)
v_ax = vFig.add_subplot(111)
for pp in range(4):
  for dd in range(4):
    i = 4*pp + dd
    v_ax.semilogx(data[i].time, EVar[i].skew, linewidth=3,
      color=colors[pp], alpha=shades[dd])

v_ax.set_xlabel("Time [ms]", fontsize=16)
v_ax.set_ylabel("Skewness of Eigenvalue Variance", fontsize=16)
...
sFig.suptitle('Skewness of Shape', fontsize=16)
s_ax = sFig.add_subplot(111)
for pp in range(4):
  for dd in range(4):
    i = 4*pp + dd
    s_ax.semilogx(data[i].time, Shape[i].skew, linewidth=3,
      color=colors[pp], alpha=shades[dd])

s_ax.set_xlabel("Time [ms]", fontsize=16)
s_ax.set_ylabel("Skewness of Shape", fontsize=16)
s_ax.legend(legendText, ncol=2,loc='lower left', framealpha=0.9)
...
iFig.suptitle('Skewness of I', fontsize=16)
i_ax = iFig.add_subplot(111)
for pp in range(4):
  for dd in range(4):
    i = 4*pp + dd
    i_ax.semilogx(data[i].time, I1[i].skew, linewidth=3,
      color=colors[pp], alpha=shades[dd])
    i_ax.semilogx(data[i].time, I2[i].skew, linewidth=3,
      color=colors[pp], alpha=shades[dd])
    i_ax.semilogx(data[i].time, I3[i].skew, linewidth=3,
      color=colors[pp], alpha=shades[dd])

i_ax.text(1.25, -0.50, 'I1', fontsize=16)
i_ax.text(1.25, 0.00, 'I2', fontsize=16)
i_ax.text(1.25, 0.50, 'I3', fontsize=16)
i_ax.set_ylabel("Skewness of I", fontsize=16)
