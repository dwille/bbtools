...
void main_read_input(void)
{
  ...
  fret = fscanf(infile, "Print Coeffs %d\n", &printCoeffsFlag);
  fclose(infile);
}
...
void domain_init(void)
{
  ...
  // Calculate order using sampling theorem -- 1 wave / diameter
  if (fourierOrder == -1) {
    fourierOrder = (int) floor(R0 / (2.*meanR));
  }
  if (legendreOrder == -1) {
    legendreOrder = 5;    //todo: samplingtheorem?
  }
...
void write_coeffs(int in, int ll, int nn)
{
  ...
  sprintf(fnameEven, "%s/%s/coeffs-r-odd-ll%d-nn%d", ANALYSIS_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
  sprintf(fnameOdd, "%s/%s/coeffs-r-even-ll%d-nn%d", ANALYSIS_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
  ...
  if (in == -1) {          // write header
    for (int lll = 0; lll <= legendreOrder; lll+=2) {
      for (int nnn = 0; nnn <= fourierOrder; nnn++) {
        fprintf(fileEven, "%d,%d ", lll, nnn);
        fprintf(fileOdd, "%d,%d ", lll, nnn);
      }
    }
    fprintf(fileEven, "\n");
    fprintf(fileOdd, "\n");
    ll = ll; nn = nn;      // prevent compiler warning

  } else if (in == 0) {    // write constant coeffs
      int c = 0.5*ll;
      fprintf(fileEven, "%lf ", g_l0[c]);
      fprintf(fileOdd, "%lf ", g_l0[c]);
      nn = nn;             // prevent compiler warnign

  } else if (in == 1) {    // write the rest
      int stride = (0.5*ll)*(fourierOrder - 1) + nn - 1;
      fprintf(fileEven, "%lf ", g_ln_even[stride]);
      fprintf(fileOdd, "%lf ", g_ln_even[stride]);

  } else if (in == 2) {   // write endline
      fprintf(fileEven, "\n");
      fprintf(fileOdd, "\n");
      ll = ll; nn = nn;      // prevent compiler warning
  }
...
void write_reconstruct(void)
{
  ...
  sprintf(fnameall2, "%s/%s/part-pair-r-%s", ANALYSIS_DIR, DATA_DIR, format);
...
void write_avg_reconstruct(void)
{
  // Set up the filename
  char fname[CHAR_BUF_SIZE] = "";
  sprintf(fname, "%s/%s/part-pair-r-avg-ll-%d-nn-%d", ANALYSIS_DIR, DATA_DIR, 
    legendreOrder, fourierOrder);
