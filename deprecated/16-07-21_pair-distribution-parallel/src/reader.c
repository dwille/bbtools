...
void read_input(void)
{
  ...
  fret = fscanf(infile, "\n");
  fret = fscanf(infile, "l Order %d\n", &orderL);
  fret = fscanf(infile, "n Order %d\n", &orderN);
...
// Initialize  output file and align.dat
void init_output(void)
{
  /* file */
  char path2file[FILE_NAME_SIZE] = "";
  sprintf(path2file, "%s/%s/stat.out", ROOT_DIR, DATA_OUT_DIR);

  // open file for reading -- stat.out
...
// Allocate memory for data_arrays
void alloc_data_arrays(void)
{
  // Shape measures
  // my_array = malloc(sizeof(double) * nTetrads)
}
...
void write_info(void)
{
  ...
  sprintf(fname, "%s/%s/output_file", ROOT_DIR, DATA_OUT_DIR);
