  ...
  dom_struct *dom, int nUnique)
{
  ...
  double varLambdaG;    // Variance of g's eigenvalues
  double detG;          // Shape
...
//  double vorticity[nDim];  // Vorticity vector
  ...
  if (tet < nUnique) {
    ...
    varLambdaG = matrixSquaredTrace3(g_hat);

    // Calculate shape of g using det(g_hat)
    detG = matrixDet3(g_hat);

    // Fill in the tetrads shape and variance factors
    tetrads[tet].eigVar = 1.5*varLambdaG*invR2*invR2;
    tetrads[tet].shape = 27.*detG*invR2*invR2*invR2;

    // Find eigenvalues, vectors of gyration tensor
    //jacobiEig3(tmp3x3, tetrads[tet].normalizedEigVals, eigVec, &nrot);

    // Normalize eigenvalues by radius of gyration
...
//      }
//    }
//    vorticity[0] = 2*abs(O[5]);
//    vorticity[1] = 2*abs(O[6]);
//    vorticity[2] = 2*abs(O[1]);
//
...
__global__ void matrixTests(void)
{
  ...
  printf("invA = %lf, %lf, %lf\n\
    ...
    %lf, %lf, %lf\n", invA[0], invA[3], invA[6],
         invA[1], invA[4], invA[7], invA[2], invA[5], invA[8]);
  printf("R = %lf, %lf, %lf\n\
 %lf, %lf, %lf,\n\
 %lf, %lf, %lf\n", R[0], R[3], R[6],
         R[1], R[4], R[7], R[2], R[5], R[8]);
